<x-layout>
    <div class="max-w-2xl mx-auto my-10 bg-white p-8 rounded-lg shadow-lg border border-gray-200">
        ...
        <form action="/students/{{ $student->id }}" method="POST" class="space-y-4" enctype="multipart/form-data">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700" for="image" class="block text-sm font-medium text-gray-700">Image</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="dob" value="{{ old('dob', $student->dob) }}" id="dob" required
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500">
                    @error('dob')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email', $student->email) }}" id="email"
                    required
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500">
                @error('email')
                    <p class="text-red-500">{{ $message }}</p>
            ...
            <div>
                ...
                <input type="text" name="phone" value="{{ old('phone', $student->phone) }}" id="phone"
                    required
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500">
                @error('phone')
                    <p class="text-red-500">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea name="address" id="address" required rows="3"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('address', $student->address) }}</textarea>
                @error('address')
                    <p class="text-red-500">{{ $message }}</p>
                @enderror
            </div>
            <div class="pt-4">
                <button type="submit"
                    class="w-full bg-indigo-600 text-white font-bold py-2 px-4 rounded-md hover:bg-indigo-700 transition duration-300">Update
                    Student</button>
            </div>
        </form>
    </div>
